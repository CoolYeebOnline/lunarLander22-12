...
static void WriteImageToBuffer(CHAR_INFO* consoleBuffer, const int charsToPrint[],
	const int coloursToPrint[], const int imageHeight, const int imageWidth, int imageXPos, int imageYPos) {
	...
	for (int y = 0; y < imageHeight; ++y)
	{
		for (int x = 0; x < imageWidth; ++x)
		{
			...
			if (coloursToPrint)
			{
				...
				consoleBuffer[(imageXPos + x) + SCREEN_WIDTH * (imageYPos + y)].Attributes = 14;
			}
